    ...
    private static void refWithReferenceQueue() {
        ...
        for (int i = 0; i < 1000000; i++) { // 创建这么多的目的是为了让内存空间紧张
            ...
            WeakReference<User> weakReference =
                new WeakReference(new User(Math.round(Math.random() * 1000)), referenceQueue);
            // 引用弱引用对象
            // 即抓起这个带有雪糕的雪糕棒
        ...
        System.out.println(
            "在不被清理的情况下，可以从WeakReference中取出对象值为：" + new WeakReference(new User(Math.round(Math.random() * 1000)),
                referenceQueue).get());
    }
}
